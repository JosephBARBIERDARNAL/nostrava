...
@Composable
fun ActivityHistoryScreen(nav: NavHostController, activity: String) {
    ...
    var bucket by remember { mutableStateOf<HistoryBucket?>(null) }

    LaunchedEffect(range, anchorMs, activity) {
        bucket = runCatching { NativeBridge.listRange(range, anchorMs, activity) }.getOrNull()
    }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        bucket?.let { b ->
            Row(
                ...
                StatBox(
                    "Distance", formatDistance(b.totalDistanceM), modifier = Modifier.weight(1f),
                    valueStyle = MaterialTheme.typography.titleMedium, contentPadding = 12.dp, centered = true,
                )
                StatBox(
                    "Time", formatDuration(b.totalMovingDurationMs), modifier = Modifier.weight(1f),
                    valueStyle = MaterialTheme.typography.titleMedium, contentPadding = 12.dp, centered = true,
                )
                StatBox(
                    config.nounPlural, b.sessionCount.toString(), modifier = Modifier.weight(1f),
                    valueStyle = MaterialTheme.typography.titleMedium, contentPadding = 12.dp, centered = true,
                )
            }
        }
        ...
        if (bucket != null && sessions.isEmpty()) {
            DiditCard {
                Text(
                    "No ${config.nounPlural.lowercase()} in this $range.",
